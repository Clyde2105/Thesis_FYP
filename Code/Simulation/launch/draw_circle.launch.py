...
def generate_launch_description():
    ...
    config_file = os.path.join(share, "config", "ros2_control.yaml")
    world_path = os.path.join(share, "worlds", "art_world.sdf")

    # FIX: Properly evaluate Xacro and publish to the /robot_description topic
    robot_state_publisher = Node(
        package="robot_state_publisher",
        executable="robot_state_publisher",
        parameters=[{
            "robot_description": Command(["xacro ", xacro_path, " ros2_control_yaml:=", config_file])
        }],
        output="screen",
    )
